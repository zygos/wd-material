...
import { FileMigrationProvider, type Kysely } from 'kysely/dist/esm'
...
async function migrateDefault(db: Kysely<any>) {
  ...
  const { results, error } = await migrateToLatest(
    // a few type hacks to interop Kysely ESM and CJS
    nodeProvider as any,
    db as any
  )
...
if (isFileRunDirectly) {
  ...
  await migrateDefault(db as any)
}
