...
export class AnthropicChatBot {
  ...
  async chat(messageHistory) {
    const messagesChained = messageHistory
      // Anthropic does not allow to use the same role for multiple messages in a row.
      // This does not allow us to chain multiple user messages. We can get around this
      // by combining multiple user messages into a single message.
      .slice(-20) // 20 last messages are enough
