...
export default async (database: Database): Promise<string> => {
  const rows = queryDatabase(database)
...
function formatValue(key: string, value: unknown) {
  ...
  // slightly hacky heuristic way to preserve non-ISO 8601 date formats
  // in the spreadsheet that are provided by applications such as Excel.
