...
it('should return all results in the CSV format', async () => {
  ...
  expect(header).toEqual(
    'package_id,package_description,weight,size,size_unit,customer_id,customer_name,driver_id,driver_name,license_number,pickup_address_id,pickup_address_name,pickup_address_full,delivery_address_id,delivery_address_name,delivery_address_full,pickup_at,delivered_at,invoice_id,invoice_serial_number,invoice_amount,currency,invoiced_at,paid_at',
  )

  // checking through json to make the test more readable, robust and independent
  // of the order of the columns once the header is checked
